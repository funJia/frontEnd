...
gulp.task('bus', function (cb) {
    console.log(1);
    runSequence('clean:' + exportDir, 'copy', 'js', 'js-html',
        'filemd5', 'filemd52')();
});

/*复制并粘贴基础公共代码 */
gulp.task('copy', function () {
    return gulp.src(['css/**', 'images/**', 'js/**',
        'student/js/artTemplate/**','teacher/js/artTemplate/**'], { base: '.' })
...
/*业务代码 */
gulp.task('js', () =>
    gulp.src('./student/js/business/*.js')
        //转换成 ES5
        .pipe(babel())
    ...
    gulp.src('./teacher/js/business/*.js')
        //转换成 ES5
        .pipe(babel())
        //合并所有js到main.jsmain.js到文件夹
        //.pipe(concat('main.js'))    
...
/*替换html */
gulp.task('js-html', () =>
    gulp.src(['./student/*.html', './student/*.aspx'])
        //转换成 ES5
        // .pipe(htmlreplace())
        //输出
        .pipe(convertEncoding({to: 'utf8'}))
        .pipe(gulp.dest(exportDir + '/student/')) && gulp.src(['./teacher/*.html', './teacher/*.aspx'])
        .pipe(convertEncoding({to: 'utf8'}))
        .pipe(gulp.dest(exportDir + '/teacher/'))
);

gulp.task('filemd5', () =>
    gulp.src([exportDir + '/student/js/business/*.js',exportDir + '/teacher/js/business/*.js'])
        // .pipe(rev())
        // .pipe(gulp.dest(exportDir + '/student/js/business'))
        // .pipe(rev.manifest())
        .pipe(rev())
...
gulp.task('filemd52', () =>
    gulp.src([exportDir + '/rev-manifest.json', exportDir + '/student/*.html', exportDir + '/student/*.aspx'])
        .pipe(revCollector({
            revSuffix: '\\?v=[0-9a-f]{8,10}-?'  //利用revCollector的可配置，去满足我们需要的模式；
        }))
        .pipe(convertEncoding({to: 'utf8'}))        
        .pipe(gulp.dest(exportDir + '/student')) &&
    gulp.src([exportDir + '/rev-manifest.json', exportDir + '/teacher/*.html', exportDir + '/teacher/*.aspx'])
        .pipe(revCollector({
            revSuffix: '\\?v=[0-9a-f]{8,10}-?'  //利用revCollector的可配置，去满足我们需要的模式；
        }))
        .pipe(convertEncoding({to: 'utf8'}))
